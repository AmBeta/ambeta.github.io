...
;(function($, window, document, undefined) {
  ...
  GenDocNav.prototype = {
    ...
    init: function() {
      ...
      if (this.elem.tagName.toLowerCase() !== 'ul') {
        console.error('Target container SHOULD be a <UL> element!');
        return;
      }
      ...
      this.$elem.onePageNav(this.options);
    },
    ...
    genListHTML: function() {
      ...
      $.each($eles, function(idx, ele) {
        list += ['<li class="' + ele.id.replace(/\-[\d]*$/gi, '') + '">', 
                    '<a href="#' + ele.id + '">', 
                      $(ele).text(), 
                    '</a>',
                  '</li>'].join('');
    ...
    genIdEles: function() {
      var id, n = 0;
      var that = this;
      return $(this.config.navTargets).each(function(idx) {
        id = (that.config.navTargetsId + '-' + this.tagName + '-' + n++).toLowerCase();
